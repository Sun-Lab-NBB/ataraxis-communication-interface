...
if __name__ == "__main__":
    ...
    # Specifies the directory in which to save all incoming and outgoing messages processed by the
    # MicroControllerInterface instance for each hardware module. Creates a temporary directory for illustration
    # purposes, and the 'with' block below removes that directory, together with everything written into it, once the
    # runtime ends. This works on the failure path as well as on the success path.
    with tempfile.TemporaryDirectory() as temporary_directory:
        ...
        # to disk. See https://github.com/Sun-Lab-NBB/ataraxis-data-structures for more details on the DataLogger class.
        ...
        # Instantiates the MicroControllerInterface. Functions similarly to the Kernel class from the
        # ataraxis-micro-controller library and abstracts most of the inner workings of the library. Expects a Teensy
        # 4.1 microcontroller, and the parameters defined below may not be optimal for all supported microcontrollers.
        ...
        # to disk.
        try:
            # Starts the serial communication with the microcontroller by initializing a separate process that handles
            # the communication. This method may take up to 30 seconds to execute, as it verifies that the
            # microcontroller is configured correctly, given the MicroControllerInterface configuration.
            microcontroller_interface.start()
            ...
            # Generates and sends new runtime parameters to both hardware module instances running on the
            # microcontroller. On and Off durations are in microseconds.
            ...
            # The wait is bounded, since an echo the microcontroller never answers would otherwise spin this loop
            # forever.
            ...
            # Cancels both recurrent commands by issuing a dequeue command. Note that the dequeue command does not
            # interrupt already running commands, it only prevents further command repetitions.
            ...
            # The pulse period is the same in both modes, so the pin is again expected to pulse ~2 times. This time the
            # pin pulsing performed by module 1 interferes with the echo command performed by module 2, so the echo
            # counter is expected to fall below the non-blocking figure above.
            ...
            # log entries to the DataLogger, and stopping the logger underneath a live writer would drop the entries
            # still in flight. The DataLogger stops second, which drains its queue and flushes every remaining entry to
            # disk as a .npy file. The archive assembly runs last, since it consolidates those .npy files into .npz
            # archives and only sees a complete set once the logger process has exited. This example writes into a
            # temporary directory that goes away with the script, so the archive is discarded on the failure path along
            # with its sources.
            microcontroller_interface.stop()
            console.echo(message="Communication process: Stopped.", level=LogLevel.SUCCESS)

            # Stops the DataLogger and assembles all logged data into a single .npz archive file. This step is required
            # to be able to extract the logged message data for further analysis.
            data_logger.stop()
        ...
        # Unlike the temporary directory managed by the 'with' block above, this output directory outlives the runtime
        # on purpose, so the extracted feather files remain available for inspection after the example exits. The
        # caller owns the directory and is responsible for removing it.
