...
def _job_execution_manager(state: JobExecutionState) -> None:
    ...
    Notes:
        ...
        Every job this manager dispatched ends in a terminal tracker state, since the tracker is the only channel
        a status reader consults. A canceled batch's queued jobs are cleared before they are dispatched, so they
        stay scheduled and remain re-runnable.
...
def _reap_finished_jobs(state: JobExecutionState) -> None:
    """Removes every finished job from the running set, records what a pool break killed, and records every job it
    reaped as one this session finished.
...
def _admit_pending_jobs(state: JobExecutionState, executor: ProcessPoolExecutor) -> None:
    ...
    for job, sizing in state.pending_jobs:
        if sizing.memory_mb > host_memory_mb:
            message = (
                f"Unable to run the microcontroller data extraction job for source '{job.source_id}' (job ID: "
