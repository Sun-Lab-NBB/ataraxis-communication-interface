...
def run_extraction_job(job: JobDescriptor) -> None:
    ...
    Notes:
        Takes one flat descriptor, so the only state crossing the process boundary is paths, strings, and integers.
    ...
    *,
    ...
    Raises:
        ...
        ValueError: If the configuration declares no entry for this controller, if it declares a module or a kernel
            ...
            path does not resolve to a readable .npz file, if the archive carries no onset timestamp message, or if a
            data message's payload size disagrees with its prototype code. Also raised if the tracker holds no entry
            for this job identifier.
