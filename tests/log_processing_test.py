...
def test_align_jobs_new_file(tmp_path: Path) -> None:
    """Verifies that align_jobs initializes a fresh tracker when no tracker file exists yet."""
...
def test_align_jobs_foreign_entries(tmp_path: Path) -> None:
    """Verifies that align_jobs resets and reinitializes the tracker when it contains foreign job entries."""
    tracker = ProcessingTracker(file_path=tmp_path / TRACKER_FILENAME)
    ...
    tracker.align_jobs(jobs=requested, universe=requested)
...
def test_align_jobs_additive(tmp_path: Path) -> None:
    """Verifies that align_jobs registers missing requested jobs without clobbering existing universe entries."""
    tracker = ProcessingTracker(file_path=tmp_path / TRACKER_FILENAME)
...
def test_align_jobs_fully_aligned(tmp_path: Path) -> None:
    """Verifies that align_jobs is a no-op when the tracker already contains every requested job."""
    tracker = ProcessingTracker(file_path=tmp_path / TRACKER_FILENAME)
    jobs = [("microcontroller_data_extraction", "1")]
